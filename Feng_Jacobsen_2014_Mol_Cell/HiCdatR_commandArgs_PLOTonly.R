...
# Usage:
# ./HiCdatR_commandArgs_PLOTonly.R 10000 10kb 100
...
# Function to plot genome profile of one dataset overlaid with other dataset 
oneVtwoGenomePlot <- function(xplot, one_norm, two_norm, three_norm, four_norm, oneYlabel, twoYlabel) {
  ...
  p <- par('usr')
  text(p[2], mean(p[3:4]), cex = 1.5, adj = c(0.5, -3.0), labels = twoYlabel, xpd = NA, srt = -90, col = "darkgreen")
...
oneVtwoGenomePlot2 <- function(xplot, one_norm, two_norm, three_norm, four_norm, oneYlabel, twoYlabel) {
  ...
  p <- par('usr')
  text(p[2], mean(p[3:4]), cex = 1.5, adj = c(0.5, -3.0), labels = oneYlabel, xpd = NA, srt = -90, col = "magenta")
...
# Function to plot genome profile of one dataset overlaid with other dataset 
oneVtwoGenomePlot_log2wtVmutant <- function(xplot, one_norm, two_norm, oneYlabel, twoYlabel) {
  plot(xplot, two_norm, type = "l", lwd = 1.5, col = "darkgreen",
       ylim = c(min(c(two_norm), na.rm = T),
                max(c(two_norm), na.rm = T)),
       xlab = "",
       ylab = "", xaxt = "n", yaxt = "n",
       main = bquote(italic("r"[s]) ~ " = " ~ .(round(cor(one_norm, two_norm, method = "spearman", use = "pairwise.complete.obs"), digits = 2))))
  axis(side = 4, at = pretty(c(two_norm)), lwd.tick = 1.5)
  p <- par('usr')
  text(p[2], mean(p[3:4]), cex = 1.5, adj = c(0.5, -2.0), labels = twoYlabel, xpd = NA, srt = -90, col = "darkgreen")
#  mtext(side = 4, line = 2.5, cex = 1, text = twoYlabel, col = "darkgreen")
  par(new = T)
  plot(xplot, one_norm, type = "l", lwd = 1.5, col = "magenta",
       ylim = c(min(c(one_norm), na.rm = T),
                max(c(one_norm), na.rm = T)),
       xlab = "",
       ylab = "")
  axis(side = 2, at = pretty(c(one_norm)), lwd.tick = 1.5)
  mtext(side = 2, line = 2.25, cex = 1, text = oneYlabel, col = "magenta")
  abline(v = sumchr, lty = 1, lwd = 0.75)
  abline(v = centromeres, lty = 5, lwd = 0.75, col = "black")
  box(lwd = 1.5)
...
oneVtwoGenomePlot(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                  ...
                  twoYlabel = "wt Hi-C eigen (inverted sign)")
oneVtwoGenomePlot(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                  ...
                  twoYlabel = "kss Hi-C eigen (inverted sign)")
wtVmutantGenomePlot(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                    ...
                    Ylabel = "Hi-C eigen (inverted sign)",
                    legendLoc = "topleft",
...
wtVmutantGenomePlot(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                    ...
                    wtCol = "blue",
                    mutantCol = "red")
oneVtwoGenomePlot_log2wtVmutant(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                                one_norm = log2((df$den_wt_REC8_HA_Rep1+5)/(df$den_kss_REC8_HA_Rep1+5)),
                                two_norm = log2((df$wt_orientedPrincpInvertSign+5)/(df$kss_orientedPrincpInvertSign+5)),
                                oneYlabel = bquote("Log"[2]*"(wt:kss) REC8-HA Rep1"),
                                twoYlabel = bquote("Log"[2]*"(wt:kss) Hi-C eigen (inverted sign)"))
dev.off()
...
wtVmutantGenomePlot(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                    ...
                    mutantCol = "red")
oneVtwoGenomePlot_log2wtVmutant(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                                one_norm = log2((df$den_wt_REC8_HA_Rep1_filt+5)/(df$den_kss_REC8_HA_Rep1_filt+5)),
                                two_norm = log2((df$wt_orientedPrincpInvertSign+5)/(df$kss_orientedPrincpInvertSign+5)),
                                oneYlabel = bquote("Log"[2]*"(wt:kss) REC8-HA Rep1"),
                                twoYlabel = bquote("Log"[2]*"(wt:kss) Hi-C eigen (inverted sign)"))
dev.off()
...
oneVtwoGenomePlot_log2wtVmutant(xplot = df$cumStart+((df$cumEnd-df$cumStart)/2),
                                one_norm = log2((df$den_wt_REC8_HA_Rep1_filt+5)/(df$den_kss_REC8_HA_Rep1_filt+5)),
                                two_norm = log2((df$wt_orientedPrincpInvertSign_filt+5)/(df$kss_orientedPrincpInvertSign_filt+5)),
                                oneYlabel = bquote("Log"[2]*"(wt:kss) REC8-HA Rep1"),
                                twoYlabel = bquote("Log"[2]*"(wt:kss) Hi-C eigen (inverted sign)"))
dev.off()
